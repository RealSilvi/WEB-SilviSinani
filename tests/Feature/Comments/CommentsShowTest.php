...
it('can fetch a single comment', function () {
    ...
    Comment::factory()->for($profile)->for($post)->create(['body' => 'commentA']);
    Comment::factory()->for($post)->create(['body' => 'commentB']);
    Comment::factory()->for($post)->create(['body' => 'commentC']);

    $response = getJson(action([CommentController::class, 'index'], [
        'user' => $user->id,
        'profile' => $profile->id,
        'post' => $post->id,
        'include' => [],
...
it('can fetch a single comment full', function () {
    $user = User::factory()->create();
    Sanctum::actingAs($user);
