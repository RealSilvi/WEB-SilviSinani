...
it('can fetch a single user account', function () {
    ...
    $response = getJson(action([AccountController::class, 'show'], [
        'user' => $user->id,
        'account' => $accountA->id,
    ]));
...
it('can fetch a single user account with chats', function () {
...
it('can fetch a single user account with chats with messages', function () {
