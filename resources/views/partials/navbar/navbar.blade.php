<div x-data="{showMenu:false}" class="fixed w-full bg-gray-100 h-15 flex items-center justify-between px-5">
    ...
    <div @click="showMenu=true">{{svg('menu-toogle','h-6 w-6')}}</div>
{{--    <form action="{{route('logout')}}" method="POST">@csrf <button>esci</button></form>--}}

    <div x-cloak x-show="showMenu" class="absolute inset-0 h-screen w-full bg-black/25">
        <div class="flex justify-end">
            <div @click.outside="showMenu=false" class="bg-gray-100 w-1/2 h-screen relative px-10 py-20">
                <div>
                    <x-navbar></x-navbar>
                </div>
